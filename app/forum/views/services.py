...
@sync_to_async
def leave_community_service(request, slug):
    user = request.user
    community = Community.objects.filter(slug=slug).first()
    
    # Delete the specific row from CommunityMembers
    CommunityMembers.objects.filter(community=community, member=user).delete()

    response = JsonResponse({"message": "Successfully left the community"})
    response["HX-Redirect"] = "/forums/explore"
    return response
