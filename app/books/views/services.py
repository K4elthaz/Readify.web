...
def save_new_book_service(request):
    try:
        ...
        if not category:
            return JsonResponse({"error": "At least one category must be selected."}, status=400)
        
        if not cover_photo:
            return JsonResponse({"error": "Cover photo is required."}, status=400)
        ...
        # If cover photo exists, upload to cloudinary
        if cover_photo:
        ...
        # Update book with uploaded cover photo URL
...
def publish_book_service(request, slug):
    user = request.user
    # Fetch the book and check its description
    book = get_object_or_404(Books, slug=slug)
    if not book.description or not book.description.strip():
        # Return an error response with an alert
        response = JsonResponse({"message": "Description cannot be empty or blank spaces"})
    ...
    response = JsonResponse({"message": "Book successfully published"})
    response["HX-Redirect"] = f"/book/detail/{slug}"
    return response
...
def delete_from_favorites(request, slug):
    ...
    if book:
        UsersFavorites.objects.filter(book=book, reader=user).delete()
        message = "Book successfully removed from favorites"
    else:
        message = "Book not found"
    
    response = JsonResponse({"message": message})
    response["HX-Redirect"] = f"/books/favorites"
    return response
